		    ...
		    const serial_conf_t* c)
{
  ...
  cfsetospeed(&termios, (speed_t)conf_to_speed_t(c));
  cfsetispeed(&termios, (speed_t)conf_to_speed_t(c));
  termios.c_cc[VMIN] = 1;
...
/*   termios.c_cflag = (termios.c_cflag & ~CSIZE) | CS8 | HUPCL; */
...
/*   termios.c_cflag &= ~CSTOPB; */
  ...
  if ((rc = fcntl(h->fd, F_GETFL, 0) != -1))
    fcntl(h->fd, F_SETFL, rc & ~O_NONBLOCK);
